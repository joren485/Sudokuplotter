...
##
lines = cv2.HoughLinesP(close, 1, np.pi/180, 2 , None, 350, 35)
lines = [list(i) for i in list(lines[0])]
##
lines = sorted(lines, key = lambda x: x[3],reverse = True)
x2 = lines[0][2]
for i in range(len(lines)):
    if lines[i][2] not in range(x2-30,x2+30):
            line10 = lines[:i]
            break
...
y2 = lines[0][3]
for i in range(len(lines)):
    if lines[i][3] not in range(y2-30,y2+30):
            line10 = lines[:i]
            break  
...
###OCR Stage
print "7) OCR Stage: "
result = [[0]*9 for number in xrange(9)]
