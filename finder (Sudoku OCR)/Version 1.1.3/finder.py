...
for row in range(9):
    ...
    for column in range(9):
        ...
        ##warp image to get digit in perfect square
        src = bm[row:row+2, column:column+2 , :].reshape((4,2))    
        dst = np.array( [ [column*square_size,row*square_size],[(column+1)*square_size-1,row*square_size],[column*square_size,(row+1)*square_size-1],[(column+1)*square_size-1,(row+1)*square_size-1] ], np.float32)
        retval = cv2.getPerspectiveTransform(src,dst)
        warp = cv2.warpPerspective(res2,retval,(size))
        
		##cut digit image out of puzzle and remove square border
        whole_digit = warp[row*square_size:(row+1)*square_size-1 , column*square_size:(column+1)*square_size-1].copy()
        noborder_digit = whole_digit[5:64,5:64]
        
        ##make the digit black and white
        ...
        ##create an actual image out of the digit data 
